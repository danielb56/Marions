...
describe("pricing isolation gate", () => {
  it("has only manager policies on both financial tables", () => {
    ...
    expect(sql).not.toMatch(
      /create policy\s+\S*worker\S*\s+on public\.(?:work_order_totals|task_pricing)/i,
    );
  ...
  it("keeps financial tables out of worker-safe views", () => {
    ...
    const views =
      sql.match(
        /create view public\.worker_[\s\S]*?comment on view public\.worker_job_safe[\s\S]*?;/i,
      )?.[0] ?? "";
    expect(views).not.toMatch(
      /task_pricing|work_order_totals|unit_rate_cents|line_total_cents|subtotal_cents|gst_cents|total_cents/i,
    );
  });

  it("does not reference financial fields from any worker page", () => {
    const source = files(join(root, "src/app/worker"))
      .filter((path) => /\.(ts|tsx)$/.test(path))
      .map((path) => readFileSync(path, "utf8"))
      .join("\n");
    expect(source).not.toMatch(
      /unit_rate_cents|line_total_cents|subtotal_cents|gst_cents|total_cents|task_pricing|work_order_totals/,
    );
  });
