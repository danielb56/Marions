...
async function main() {
  ...
  let { data: client } = await admin
    ...
    .ilike("name", "Bentino Pty Ltd")
    .limit(1)
    .maybeSingle();
  if (!client) {
    const inserted = await admin
      .from("client")
      .insert({ tenant_id: tenantId, name: "Bentino Pty Ltd" })
      .select("id")
      .single();
  ...
  const { data: customer } = await admin
    .from("customer")
    .insert({ tenant_id: tenantId, name: "Francesco Amato", phone: "0409735370" })
    .select("id")
    .single();
  ...
  await admin.from("site_contact").insert({
    tenant_id: tenantId,
    site_id: site.id,
  ...
  const { data: order, error: orderError } = await admin
    ...
    .insert({
      ...
      site_id: site.id,
      work_order_number: "20299-29572",
      job_number: "20299",
      client_reference: "12201037101",
      client_supervisor_name: "Astafanos Kheir",
      status: "ready",
      additional_instructions:
        "Confirm access with the site contact before arrival. Protect finished surfaces and leave the site clean.",
  ...
  for (const [tradeName, area, description, quantity, unit] of tasks) {
    position += 1;
    const trade = trades?.find((item) => item.name === tradeName);
    if (!trade) continue;
  ...
  await admin.from("work_order_totals").insert({
    work_order_id: order.id,
    tenant_id: tenantId,
