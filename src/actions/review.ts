...
export async function reviewSubmission(_: ActionState, formData: FormData): Promise<ActionState> {
  ...
  if (!submissionId || !["approved", "changes_requested", "rejected"].includes(decision))
    return { error: "Choose a review decision." };
  if (decision !== "approved" && !notes.trim())
    return { error: "A note is required when returning or rejecting work." };
  const supabase = await createClient();
  const { error } = await supabase.rpc("manager_review_submission", {
  ...
  return {
    ok: true,
    message: decision === "approved" ? "Completion approved." : "Work returned to the worker.",
  };
}
