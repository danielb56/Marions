...
export function TaskEditForm({ task }: TaskEditFormProps) {
  ...
  return (
    <details className="mt-4 rounded-xl border border-[#d8e3e1] bg-[#f5fafc] p-3">
      ...
      <form
        action={action}
        className="mt-4 grid gap-3 md:grid-cols-[minmax(0,1fr)_minmax(10rem,0.35fr)_7rem_6.5rem]"
      >
        ...
        <div>
          ...
          <Input
            ...
            name="area"
            maxLength={120}
            defaultValue={task.areaLabel ?? ""}
            placeholder="For example, Lounge"
          />
          {state.fieldErrors?.area?.map((error) => (
            <p key={error} className="mt-1 text-xs text-[#913a31]">
              {error}
            </p>
          ))}
        ...
        <div>
          ...
          />
          {state.fieldErrors?.quantity?.map((error) => (
            <p key={error} className="mt-1 text-xs text-[#913a31]">
              {error}
            </p>
          ))}
        ...
        <div>
          ...
          {state.fieldErrors?.unit?.map((error) => (
            <p key={error} className="mt-1 text-xs text-[#913a31]">
              {error}
            </p>
          ))}
        ...
        <div className="flex items-end">
          <SubmitButton className="w-full" pendingText="Saving task...">
            Save changes
          </SubmitButton>
        </div>
        {state.error && (
          <p role="alert" className="text-sm text-[#913a31] md:col-span-4">
            {state.error}
          </p>
        )}
        {state.message && (
          <p role="status" className="text-sm font-semibold text-[#2f6249] md:col-span-4">
            {state.message}
          </p>
        )}
      </form>
