...
import { CalendarX, TriangleAlert, UserMinus } from "lucide-react";
import { unassignAllUnscheduled, unassignTask, unscheduleAllUpcoming } from "@/actions/work-orders";
...
// Clears every upcoming date but keeps the assignments, so the jobs drop back
// into Assigned but unscheduled rather than becoming unassigned work.
    ...
    <details className="relative">
      ...
      >
        <div className="flex gap-2 rounded-xl bg-[#f5dfdc] p-3 text-sm text-[#7b302a]">
          ...
          <p>
            This removes every upcoming date from the calendar. The jobs stay assigned to their
            workers and move into Assigned but unscheduled, and each affected worker is notified
            once.
          </p>
        </div>
        <div>
          ...
          <Input
            id="unschedule-all-reason"
            name="reason"
            required
            minLength={2}
            maxLength={500}
            placeholder="Why is the schedule being cleared?"
          />
        </div>
        {state.error && (
        ...
        <SubmitButton className="w-full" variant="danger" pendingText="Clearing schedule...">
          Confirm unschedule all
        </SubmitButton>
      </form>
    </details>
  );
...
export function UnassignAllControl() {
  const [state, action] = useActionState(unassignAllUnscheduled, {} as ActionState);
