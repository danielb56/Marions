...
export function SettingsForm({ tenant }: { tenant: TenantSettings }) {
  ...
  return (
    <form action={action} className="space-y-5">
      ...
      <div>
        <Label>ABN</Label>
        <Input name="abn" defaultValue={tenant.abn ?? ""} />
      </div>
      <div>
        ...
        <Input
          ...
          min="1"
          max="20"
          defaultValue={tenant.retention_years}
        />
      </div>
      <label className="flex min-h-12 items-center justify-between gap-4 rounded-xl border border-[#ddd8ce] p-3.5 text-sm font-semibold">
        <span>Require MFA for managers</span>
        <input
          type="checkbox"
          name="mfaRequired"
          defaultChecked={tenant.mfa_required_for_managers}
          className="h-5 w-5 accent-[#0077a8]"
        />
      </label>
      <label className="flex min-h-12 items-center justify-between gap-4 rounded-xl border border-[#ddd8ce] p-3.5 text-sm font-semibold">
        <span>Enable assignment SMS</span>
        <input
          type="checkbox"
          name="smsEnabled"
          defaultChecked={tenant.sms_enabled}
          className="h-5 w-5 accent-[#0077a8]"
        />
      </label>
