...
type ScheduleRow = { id: number; start_time: string | null; task: { id: number; description: string; status: TaskStatus; work_order: { id: number; work_order_number: string; client: { name: string } | null; site: { suburb: string } | null } | null } | null; worker: { user_profile: { display_name: string } | null } | null };
...
export default async function ManagerDashboard() {
  ...
  const [ordersResult, pendingResult, tasksResult, scheduleResult, auditResult, revenueResult] = await Promise.all([
    ...
    supabase.from("schedule_entry").select("id,start_time,task:task_id(id,description,status,work_order:work_order_id(id,work_order_number,client:client_id(name),site:site_id(suburb))),worker:worker_id(user_profile:user_id(display_name))").eq("planned_date", today).order("start_time"),
  ...
  const cards = [
    { label: "Active work orders", value: ordersResult.count ?? 0, icon: ClipboardList, tone: "bg-[#dcebea] text-[#2d686e]", href: "/manager/work-orders" },
  ...
  return <>
    ...
    <section className="mt-6 grid gap-6 xl:grid-cols-[1.45fr_.85fr]">
      <Card className="overflow-hidden"><div className="flex items-center justify-between border-b border-[#ebe7df] px-5 py-4"><div><h2 className="font-semibold">Today&apos;s schedule</h2><p className="text-sm text-[#77807e]">{formatDate(today, "EEEE, d MMMM")}</p></div><Link href="/manager/calendar" className="text-sm font-semibold text-[#2f666c]">Open calendar</Link></div><div className="divide-y divide-[#ebe7df]">{schedule.length ? schedule.map((item) => <Link key={item.id} href={`/manager/work-orders/${item.task?.work_order?.id}`} className="grid grid-cols-[64px_1fr_auto] items-center gap-3 px-5 py-4 hover:bg-[#faf9f6]"><span className="font-mono text-sm font-semibold text-[#596461]">{item.start_time?.slice(0,5) ?? "Any"}</span><div><p className="line-clamp-1 font-semibold">{item.task?.description}</p><p className="mt-0.5 text-sm text-[#75807c]">{item.task?.work_order?.work_order_number} · {item.task?.work_order?.client?.name} · {item.task?.work_order?.site?.suburb}</p><p className="mt-1 text-xs font-medium text-[#a46327]">{item.worker?.user_profile?.display_name ?? "No worker"}</p></div>{item.task?.status && <StatusBadge status={item.task.status} />}</Link>) : <div className="px-5 py-12 text-center"><CalendarClock className="mx-auto h-7 w-7 text-[#9ba29f]" /><p className="mt-3 font-semibold">Nothing scheduled today</p><p className="mt-1 text-sm text-[#7a8380]">Use the calendar to plan the crew.</p></div>}</div></Card>
      ...
      <Card className="overflow-hidden"><div className="flex items-center justify-between border-b border-[#ebe7df] px-5 py-4"><h2 className="font-semibold">Recent activity</h2><Link href="/manager/audit" className="text-sm font-semibold text-[#2f666c]">View all</Link></div><div className="divide-y divide-[#ebe7df]">{audit.map((event) => <div key={event.id} className="flex gap-3 px-5 py-3.5"><div className="mt-1 h-2 w-2 shrink-0 rounded-full bg-[#5b8e91]" /><div className="min-w-0"><p className="truncate text-sm font-medium">{event.actor?.display_name ?? "System"} · {event.action.replaceAll("_", " ").replace(".", " ")}</p><p className="mt-0.5 text-xs text-[#7d8583]">{timeAgo(event.created_at)}</p></div></div>)}{!audit.length && <p className="px-5 py-8 text-center text-sm text-[#7a8380]">Activity will appear here.</p>}</div></Card></div>
    </section>
    <Link href="/manager/work-orders?attention=unassigned" className="mt-6 flex items-center justify-between rounded-2xl bg-[#173f45] px-5 py-4 text-white"><span className="font-semibold">Review work needing attention</span><ArrowRight className="h-5 w-5" /></Link>
  </>;
}
