...
export default async function WorkOrdersPage({ searchParams }: { searchParams: Promise<Record<string, string | undefined>> }) {
  ...
  <Card className="overflow-hidden"><form className="flex flex-col gap-3 border-b border-[#e8e4dc] p-4 sm:flex-row"><div className="relative flex-1"><Search className="absolute left-3.5 top-3.5 h-4 w-4 text-[#89918e]" /><input name="q" defaultValue={query} placeholder="Search order or client reference" className="min-h-11 w-full rounded-xl border border-[#d9d4c9] bg-white pl-10 pr-3 text-sm outline-none focus:border-[#3b7379]" /></div><select name="status" defaultValue={status} className="min-h-11 rounded-xl border border-[#d9d4c9] bg-white px-3 text-sm"><option value="">All statuses</option>{["ready","assigned","scheduled","in_progress","changes_requested","blocked","signed_off","cancelled"].map((value) => <option key={value} value={value}>{value.replaceAll("_", " ")}</option>)}</select><button className="min-h-11 rounded-xl border border-[#d9d4c9] bg-[#f8f6f1] px-4 text-sm font-semibold">Filter</button></form>
